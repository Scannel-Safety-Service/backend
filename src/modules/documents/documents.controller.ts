...
export class DocumentsController {
  ...
  async findAll(@Query() queryDto: DocumentQueryDto) {
    const result = await this.documentsService.findAll(queryDto);
    return {
  ...
  async update(
    ...
    const document = await this.documentsService.update(id, updateDocumentDto, file);
    return {
  ...
  async restore(@Param('id') id: string) {
    const document = await this.documentsService.restore(id);
    return {
  ...
  async permanentDelete(@Param('id') id: string) {
    await this.documentsService.permanentDelete(id);
  }
}
